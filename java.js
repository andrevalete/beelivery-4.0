...
function applyTheme() {
  ...
  document
    .querySelectorAll("#themeToggleTop,#themeToggleCat,#themeToggleCart")
    .forEach((b) => {
      if (!b) return;
...
function updateCounters() {
  ...
  document
    .querySelectorAll("#countTop,#countCat,#topCount,#catCount")
    .forEach((el) => {
      ...
      item.name
        ...
        <div style="flex:1">
          ...
          <div style="margin-top:8px">
            ...
            <button data-rem="${
              item.id
            }" class="btn" style="margin-left:12px">Remover</button>
          </div>
        </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const openCatalogButtons = document.querySelectorAll(
    "#abrirCatalogoTop,#btnPedirTop,#abrirCatalogoTop"
  );
  openCatalogButtons.forEach((b) => {
    if (!b) return;
    b.addEventListener("click", () => window.open("catalogo.html", "_self"));
  });
  ...
  if (document.getElementById("productsGrid")) {
    ...
    if (search)
      search.addEventListener("input", () =>
        renderProducts(search.value, filter ? filter.value : "all")
      );
    if (filter)
      filter.addEventListener("change", () =>
        renderProducts(search ? search.value : "", filter.value)
      );
    ...
    document.getElementById("productsGrid").addEventListener("click", (ev) => {
      ...
      if (view) {
        ...
        const p = PRODUCTS.find((x) => x.id === id);
        if (p) toast(p.name + " — R$ " + p.price.toFixed(2));
      }
    });
